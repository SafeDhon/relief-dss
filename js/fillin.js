...
function getArray() {
  const selectedLocationsData = sessionStorage.getItem("selectedLocationsData");
...
function getArrayWithVictim() {
  const rowData = [];
  const rows = document.querySelectorAll("tbody tr");
  rows.forEach((row, index) => {
    const firstNumberInput = row.querySelector(
      'td:nth-child(2) input[type="number"]'
    );
    if (firstNumberInput) {
      const id = index - 1;
      const inputValue = parseInt(firstNumberInput.value);
      if (!isNaN(inputValue)) {
        rowData.push({
          id: id,
          victim: inputValue,
