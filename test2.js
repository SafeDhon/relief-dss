require([
  ...
  // ข้อมูลจุดทั้ง 4 จุด
  stopData = [
    { id: 1, lat: 13.7563, long: 100.5018, address: "Bangkok", victim: 0, bags: 0 }, // จุดเริ่มต้น
    { id: 2, lat: 13.7367, long: 100.5231, address: "Silom", victim: 12, bags: 12 },
    { id: 3, lat: 13.7450, long: 100.5345, address: "Pathumwan", victim: 8, bags: 8 },
    { id: 4, lat: 13.7680, long: 100.5100, address: "Dusit", victim: 5, bags: 5 },
  ];
  ...
  // เมื่อการวาดเสร็จสิ้น
  sketchVM.on("create", async (event) => {
    if (event.state === "complete") {
      const newPolyline = event.graphic.geometry;
      // เพิ่ม polyline geometry ที่ผู้ใช้วาดเข้าไปใน array
      customPolylineBarriers.push(
        new PolylineBarrier({
          geometry: newPolyline,
          name: `Custom Polyline Barrier ${customPolylineBarriers.length + 1}`,
        })
      );
    }
  });
  ...
  document.getElementById("recalculateRoute").addEventListener("click", () => {
    console.log("click");
    // console.log(customPolylineBarriers);
    calculateAndDisplayAllRoutes();
  });
});
