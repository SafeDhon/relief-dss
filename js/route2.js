require([
  ...
  for (let i = 0; i < routeIds.length - 1; i++) {
    ...
    routeSteps.push({
      order: i + 1,
      from: fromStop.address,
      to: toStop.address,
      distance_km: (dist / 1000).toFixed(2), // ✅ km, 2 ตำแหน่ง
      cumulative_km: (cumulative / 1000).toFixed(2), // ✅ km, 2 ตำแหน่ง
    });
  }
  ...
  function renderRouteTable(steps) {
    ...
    ["ลำดับ", "ต้นทาง", "ปลายทาง", "ระยะทาง (กม.)", "ระยะทางสะสม (กม.)"].forEach(
      (text) => {
