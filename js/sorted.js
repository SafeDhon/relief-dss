  ...
  btn.addEventListener("click", () => {
    ...
    Object.entries(routeData).forEach(([vehicleId, groups]) => {
      ...
      groups.forEach((arr, index) => {
        ...
        const url =
          vehicleId === "4"
            ? `../pages/route2.html?wing=${wing}&data=${data}`
            : `../pages/route1.html?wing=${wing}&data=${data}`;

        // ใช้ setTimeout เว้นช่วง
        setTimeout(() => {
          window.open(url, "_blank");

          opened++;
          // ✅ ถ้าเปิดครบทุกแท็บแล้ว ค่อย redirect
          if (opened === totalTabs) {
            setTimeout(() => {
              window.location.href = "../pages/list.html";
            }, 500); // เว้นอีกนิดให้แท็บสุดท้ายขึ้นจริง ๆ
          }
        }, index * 500);
      });
    });
